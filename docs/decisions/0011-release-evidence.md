...
- **Open:** publish validator results for every supported runtime showing all valid fixtures pass and every invalid fixture fails for its intended reason.
- **Open:** publish an interoperability report from two independent implementations covering `core`, `graph`, `media`, and `action` (unless another decision removes a profile).
- **Open:** complete and approve a security/privacy review covering archive extraction, exhaustion, execution, links, extensions, and misleading trust/authority claims. [SECURITY.md](../../SECURITY.md) and consumer vectors are inputs, not recorded approval.
- **Open:** record governance approval of normative text, schemas, both licenses, changelog, known limitations, and all ten preceding gates.
