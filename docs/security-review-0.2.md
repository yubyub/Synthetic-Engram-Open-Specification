...
byte is attacker-controlled. The included pilot processors exercise the format;
they are not hardened importers.
...
- **Accept** — Core 0.2 contains an adequate interoperability rule; applications
...
| Release disposition | **Accept with limitation; block implementation claim without evidence.** REQ-SEC-002 is mandatory, but Core 0.2 deliberately does not prescribe one unsafe-for-some or unusable-for-others global quota. |
...
| Residual risk | The pilot processors report non-execution but are not real renderers, so they do not prove sanitizer correctness. Sanitization is context-specific and libraries evolve; safe HTML output is not automatically safe in attributes, URLs, shells, terminals, or logs. |
...
| Current mitigation | Attachment size and SHA-256 are checked against bytes. The normative security rule and repository policy explicitly limit hashes to integrity/change detection and place signing, identity proof, and authorization outside Core 0.2. |
...
| Residual risk | JSON-compatible extension values are recursively unconstrained in semantic meaning and can be large/deep. The pilot exercise does not prove preservation for every serializer or extension shape. Rejecting future profile names limits forward compatibility, while accepting them outside this schema risks silent loss. |
| Pilot disposition | **Accept with limitation.** Core 0.2 rejects schema-unknown profiles; known-but-unsupported declared profiles require a deterministic unsupported result. Unknown extensions are opaque data, not trusted instructions. |
...
| Affected requirements | REQ-INV-002/003/005, REQ-CLOSE-001/003/005, REQ-MEDIA-001/002, REQ-CONF-002, REQ-SEC-001/002/005. Atomic commit is not otherwise standardized by Core 0.2. |
...
| Pilot disposition | **Accept with limitation; block importer deployment without manual evidence.** Atomic import is a necessary product safety property but is outside the interchange semantics. Implementations must stage, validate, authorize, and commit as separate phases, failing closed before publication. |

## Pilot conclusion

No reviewed item blocks publication for controlled pilot development. The model has clear
...
For pilot evidence, retain validator output, processor harness reports, and the
product-specific manual records identified above. Passing the repository suite
is useful implementation evidence, but it is not independent interoperability
evidence or a security certification of an extractor, renderer, network
fetcher, or importer.
