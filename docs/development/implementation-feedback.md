...
The sequenced delivery plan for this register is
[`implementation-plan.md`](implementation-plan.md); its maintained execution
status is in [`implementation-backlog.md`](implementation-backlog.md).
...
| FB-009 | Restricted YAML is safer but burdens every parser implementer. | Publish a standalone Engram Front Matter grammar/test corpus, assess JSON front matter, and provide maintained parsing libraries and editor linting. | [Grammar and parser contract](../front-matter.md), shared corpus, cross-runtime differential/property tests, SDK APIs, fuzzing, and editor diagnostics. | documented; planned |
| FB-010 | Custom prefixed ULIDs complicate mapping existing UUIDs, URIs, and changing types. | Document native-ID aliases, import mapping, reclassification, merge/split, collisions, and URI identity; evaluate an optional global identifier. | [Identity mapping guide](../identity-lifecycle.md), executable lifecycle fixtures, independent results, and a future decision record if the schema changes. | documented; evidence needed |
| FB-011 | Engram, export, and package identities are logical but easy to confuse. | Add lifecycle examples for first export, retry, repack, later snapshot, partial export, and re-export, including logging and deduplication advice. | [Identity lifecycle guide](../identity-lifecycle.md) and executable Python/Node vectors for retries, repacking, snapshots, partial exports, and re-exports. | complete |
...
| FB-015 | Provenance and history are major gaps for human–AI knowledge. | Prioritize a small provenance profile mapping to PROV concepts before attempting a complete synchronization model; distinguish authored, generated, derived, corrected, and adopted knowledge. | [Provenance exploration](provenance-profile-proposal.md) now; decision record, schemas, fixtures, and two independent implementations before a profile is accepted. | documented; planned |
| FB-016 | Graph interoperability is only topology and labels. | Market it explicitly as portable directed topology and publish loss-aware mappings to JSON-LD/RDF or established graph formats rather than inventing a broad ontology. | [Loss-aware mapping contracts](../graph-mappings.md) now; executable round-trip reports and experienced community review still needed. | documented; evidence needed |
| FB-017 | A stable standard can be confused with production-hardened reference tooling. | Label the specification, validator, adapters, SDKs, and security certification statuses separately. | [Component maturity matrix](../status.md), README/tool README wording, and the release-checklist maturity note distinguish every component. | complete |
| FB-018 | Existing standards cover important adjacent layers. | Maintain concrete, versioned, loss-aware mappings to BagIt, OCFL, RO-Crate, JSON-LD/RDF, PROV-O, iCalendar/VTODO, and an MCP delivery pattern. | [Preservation plans](../preservation-mappings.md), graph/PROV guidance, and [remote-delivery pattern](../remote-delivery-pattern.md) now; fixtures, reports, and community review still needed. | documented; planned |
