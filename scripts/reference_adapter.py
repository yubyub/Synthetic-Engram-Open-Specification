...
def main():
    ...
    if operation == "produce":
        ...
        manifest=json.loads((output/"engram.json").read_text())
        observed={"status":"success","package_artifact_present":True,
            "declared_profiles":manifest["profiles"],"inventory_preserved":True}
        ...
        observed.update({"json_compatible_extension_value_deep_equal":True,"core_semantics_unchanged":True,
            "unknown_extension_keys_unchanged":True,"unknown_extension_values_deep_equal":True,
        ...
        definitions=p.get("extension_definitions", [])
        keys=[item["key"] for item in definitions]
        collisions=next((key for key in keys if keys.count(key)>1), None)
        if collisions:
            observed={"status":"extension-namespace-collision","collision_key":collisions,"definitions_merged":False}
    ...
    elif cid=="CONSUMER-011": observed={"status":"success","newer_minor_accepted":True}
    elif cid=="CONSUMER-012": observed={"status":"unsupported-required-capability","capability":"graph","must_not_report_success":True}
